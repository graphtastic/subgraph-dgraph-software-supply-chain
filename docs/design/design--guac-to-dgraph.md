...
| :--- | :--- | :--- |
| **GraphQL Hive** | A schema registry for managing federated graphs, including features for monitoring, analytics, and detecting breaking changes. | [the-guild.dev/graphql/hive](https://the-guild.dev/graphql/hive) |
| **GraphQL Mesh** | A library for creating a unified GraphQL API from multiple, disparate sources like REST, gRPC, or other GraphQL schemas. Can act as a federated gateway. | [the-guild.dev/graphql/mesh](https://the-guild.dev/graphql/mesh) |
| **GraphQL Code Generator** | A tool that generates code (e.g., TypeScript types, React hooks) from a GraphQL schema and operations, ensuring type safety. | [the-guild.dev/graphql/codegen](https://the-guild.dev/graphql/codegen) |
| **GraphQL Tools** | A set of low-level, modular utilities for building, stitching, and mocking GraphQL schemas in a schema-first approach. | [the-guild.dev/graphql/tools](https://the-guild.dev/graphql/tools) |
| **GraphQL Inspector** | A tool for validating schemas, detecting breaking changes, and ensuring consistency between schemas and client-side operations. | [the-guild.dev/graphql/inspector](https://the-guild.dev/graphql/inspector) |
| **GraphQL Yoga** | A fully-featured, performant, and extendable GraphQL server for Node.js, often used to serve schemas created with GraphQL Tools. | [the-guild.dev/graphql/yoga-server](https://the-guild.dev/graphql/yoga-server) |
...
| **Schema File Path** | `--schema-file` | `GUAC_TO_DGRAPH_SCHEMA_FILE` | Path to a local SDL file. If not provided, the tool will use introspection. | *(optional)* |
